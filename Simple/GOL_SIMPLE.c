...
void swapPlains(char*** prevPlain, char*** currPlain)
{
  char **suppPlain;
...
double calculateTime(struct timeval start, struct timeval stop){
  double first_difference = (double)(stop.tv_usec - start.tv_usec);
  double second_difference = (double)(stop.tv_sec - start.tv_sec);
  return first_difference / 1000000 + second_difference;
}

//useful for debugging with small plains
void printPlain(char** givenPlain, int rows, int columns){
  int i, j;
...
bool plainsDifferent(int rows, int columns, char **prevPlain, char **currPlain)
{
  int i, j;
